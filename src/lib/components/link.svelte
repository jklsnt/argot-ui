...
<div style="border: 1px dotted gray; margin: 10px 0; padding: 15px;">
    ...
    <div style="float:right; font-size: 1; display: flex; flex-direction: column; gap: 3px; z-index: 10000; position: relative;">	
        ...
        <span style="font-size: 13px;"><a style={"font-size: 13px; min-height: 20px; z-index: 100000; display:"+(user.name == obj.author ? "inline-block;":"none;")} on:click={() => {
                alert("na na goon");
                }} href={"javascript:void(0)"}>delete</a> <span style={"display:"+(user.name == obj.author ? "inline-block;":"none;")}>|</span> <a style="font-size: 13px; min-height: 20px; display: inline-block;z-index: 100000" href={`/posts/${obj.id}`}>discuss</a> | {obj.time}</span>
    </div>
